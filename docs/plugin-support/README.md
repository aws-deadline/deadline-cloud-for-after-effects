...
3. The `RSMB` installer will add 3 `.aex` files to `C:\Program Files\Adobe\Common\Plug-ins\7.0\MediaCore\RSMB6AE`. Zip these up into `RSMB_6_4_4_aex.zip`.
If you don't have an archive manager in Windows, you can select the plugin files in Explorer, right-click them, and use `Send To > Compressed (zipped) folder`.

4. Generate the `sha256` hash for this file with the following command in a PowerShell terminal: 
   
   ```
   (Get-FileHash -Path "RSMB_6_4_4_aex.zip" -Algorithm SHA256).Hash.ToLower()
   ```
   
5. In the recipe's `rsmb-6-4-4/meta.yaml` file, update the `source > sha256` field with the hash output from above.
   
6. Send the build job to the farm with the following command inside the `conda_recipes` folder:
   
   ```
   submit-package-job rsmb-6.4.4 -q "Your queue name"
   ```
   
   If encountering login or specified queue errors:
...
7. A build job should now be running on your farm. Once completed, you can view your package by navigating to your S3 bucket, with its default location:
...
When working with licensed plugins like RSMB, a floating network license is required. For consistent performance and security, you can use AWS Session Manager to get cloud workers connected to your networked license server. The license server can run anywhere: in a VPC or over the Internet.
